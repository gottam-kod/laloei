...
const InviteScreen: React.FC<Props> = ({
  ...
  const safeOnInvite = onInviteMembers ?? (async () => {});   // default
  ...
  const link = useMemo(() => {
    ...
    const origin = 'https://laloei.app/invite';
     ...
     <SafeAreaView style={{ flex: 1, backgroundColor: '#F7FAFD' }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 160 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <Pressable onPress={() => setShowRolePicker(true)} style={styles.selectBtn}>
            <Text style={styles.selectTxt}>{ROLES.find(r => r.code === role)?.label ?? role}</Text>
      ...
      <Modal visible={showRolePicker} transparent animationType="fade" onRequestClose={() => setShowRolePicker(false)}>
        ...
        <View style={styles.modalSheet}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                onPress={() => { setRole(item.code); setShowRolePicker(false); }}
